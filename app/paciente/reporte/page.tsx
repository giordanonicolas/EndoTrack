 ...
 * Server Component: consulta y procesa pain_records en el servidor.
 * RLS + filtro explícito garantizan que solo se muestran datos de la paciente autenticada.
 *
...
/** Cuenta frecuencia de cada valor en un array y devuelve los N más comunes. */
...
function SectionCard({
  ...
  return (
    <section className="rounded-2xl bg-white border border-rose-100 shadow-sm p-6 flex flex-col gap-3">
...
export default async function ReportePage() {
  ...
  // Consulta de registros — RLS + filtro explícito
  ...
  const factoresFrec = frecuencia(
    records.flatMap((r) => r.associated_with ?? [])
  );
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-6 py-8 flex flex-col gap-6">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <Link
            ...
            className="shrink-0 rounded-full border border-rose-200 px-4 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-50 transition-colors"
          >
            Ver historial
          </Link>
        </div>
        ...
        {total === 0 ? (
          <div className="rounded-2xl bg-white border border-rose-100 shadow-sm p-10 flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-slate-600 font-medium">
              Todavía no hay registros suficientes para generar un reporte.
            </p>
            <p className="text-sm text-slate-400 max-w-xs">
              Registrá al menos un episodio de dolor para ver el reporte aquí.
            </p>
            <Link
          ...
          <>
            ...
            {descripcionesConfirmadas.length > 0 && (
              <SectionCard title="Descripciones confirmadas para consulta">
        ...
        <p className="text-xs text-slate-400 text-center pb-4">
          Solo vos podés ver este reporte. EndoTrack no comparte tu información
